                ...
                if(strstr(gps.buf,"$GPRMC")!=NULL)
                {
                    if ((nmea_parse(&parser, (char*)gps.buf, (int)strlen((char*)gps.buf), &info)) > 0 )
                    {
                        ...
                        */
                        GUI_GotoXY(0,16);
                        GUI_DispString("date:");
                        GUI_DispDec(info.utc.year+1900,4);
                        GUI_DispString("/");
                        GUI_DispDec(info.utc.mon,2);
                        GUI_DispString("/");
                        GUI_DispDec(info.utc.day,2);

                        GUI_GotoXY(0,32);
                        GUI_DispString("time:");
                        GUI_DispDec(info.utc.hour,2);
                        GUI_DispString(":");
                        GUI_DispDec(info.utc.min,2);
                        GUI_DispString(":");
                        GUI_DispDec(info.utc.sec,2);
